...
func (a *App) HandleIndex(w http.ResponseWriter, r *http.Request) {
    ...
    type Row struct { ID int64; Title, Body, Username string; CreatedAt string; Likes, Dislikes int }
    var posts []Row
    for rows.Next() {
        var rrow Row
        var ts time.Time
        if err := rows.Scan(&rrow.ID, &rrow.Title, &rrow.Body, &ts, &rrow.Username, &rrow.Likes, &rrow.Dislikes); err != nil { http.Error(w, err.Error(), 500); return }
...
func (a *App) HandleRegister(w http.ResponseWriter, r *http.Request) {
    ...
    case http.MethodPost:
        ...
        if err != nil {
            if isUniqueErr(err) { http.Error(w, "email or username already taken", http.StatusConflict); return }
...
func (a *App) HandleLogin(w http.ResponseWriter, r *http.Request) {
    ...
    case http.MethodPost:
        ...
        if bcrypt.CompareHashAndPassword([]byte(hash), []byte(pass)) != nil { http.Error(w, "invalid credentials", http.StatusUnauthorized); return }
        sid := uuid.NewString()
...
func (a *App) HandleLogout(w http.ResponseWriter, r *http.Request) {
    c, err := r.Cookie(a.CookieName)
    if err == nil { a.DB.Exec(`DELETE FROM sessions WHERE id=?`, c.Value) }
...
func (a *App) HandleNewPost(w http.ResponseWriter, r *http.Request) {
    ...
    case http.MethodPost:
        ...
        if title == "" || body == "" { http.Error(w, "title/body required", 400); return }
            ...
            for _, cname := range r.Form["category"] {
                for _, part := range strings.Split(cname, ",") {
                    ...
                    if errors.Is(err, sql.ErrNoRows) {
                        ...
                        if err == nil { cid, _ = res.LastInsertId() }
                    }
                    if cid != 0 {
...
func (a *App) HandleShowPost(w http.ResponseWriter, r *http.Request) {
    ...
    type Post struct { ID int64; Title, Body, Username string; CreatedAt string; Likes, Dislikes int; Categories []string }
    var p Post
    var ts time.Time
    ...
    defer rows.Close()
    for rows.Next() { var n string; rows.Scan(&n); p.Categories = append(p.Categories, n) }
    ...
    if err != nil { http.Error(w, err.Error(), 500); return }
    type Comment struct { ID int64; Body, Username, CreatedAt string; Likes, Dislikes int }
...
func (a *App) HandleNewComment(w http.ResponseWriter, r *http.Request) {
    ...
    body := r.FormValue("body")
    if body == "" { http.Error(w, "empty comment", 400); return }
